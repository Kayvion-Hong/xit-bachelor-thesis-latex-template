...
为方便不同网络环境下的访问与下载，本项目同时发布在 [GitHub](https://github.com/Kayvion-Hong/xit-bachelor-thesis-latex-template) 和 [Gitee](https://gitee.com/G1257314839/xit-bachelor-thesis-latex-template)。如果其中一个站点打开或下载较慢，可以尝试另一个地址，按自己的访问体验选择即可。
...
**对于第一次使用在线 LaTeX 的同学，本教程建议优先尝试 LoongTeX（龙文）。** 中文界面与帮助文档便于对照学习，也方便你先确认当前网络下的编辑和编译体验。若导师或同学已经使用 Overleaf，继续使用熟悉的平台同样可以，方便交流与协作。

- **优先尝试 [LoongTeX（龙文）][loongtex]**：可从官网进入[工作台][loongtex-app]，按下方“在 LoongTeX 中打开模板”完成上传和设置。
- **[Overleaf][overleaf] 也是可选平台**：本页保留完整入门步骤及官方图文帮助。不同网络环境和访问时段下，页面加载或操作响应可能有所差异；若偶尔需要多等待一会儿，可以稍后重试，或选择当前访问更顺畅的平台。

这里的优先顺序是面向新手的入门建议，并非两个平台的速度测试结论。无论选择哪一个，都先用本模板完成一次“修改姓名—编译—检查 PDF”，再开始正式写作。
